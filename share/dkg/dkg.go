...
type DistKeyGenerator struct {
	...
	// list of commitments to each secret polynomials
	commitments map[uint32]*share.PubPoly

	// map of deals collected to reconstruct the full polynomial of a dealer
	// key is index of the dealer. Once there is enough ReconstructCommits
	// struct, the dkg will re-construct the polynomial and stores it into
...
// ProcessResponse takes a response from every other peers.  If the response
...
// participants and must be processed by ProcessSecretCommits. In this manner,
...
// ProcessSecretCommits takes a SecretCommits from every other participants and
...
// participants. In returns nil,nil otherwise.
...
// ReconstructCommits message that must be  broadcasted to every other participants
...
// along any others. If there is enough message to recover the coefficients of
