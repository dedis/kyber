...
func TestTest(t *testing.T) {
	...
	sU := &proto.Test{}
	err = pb.Unmarshal(data, sU)
	require.NoError(t, err)

	require.Equal(t, val, *sU.Value)
	// 100101100 (=150)
	// 100101011 (=299)
	...
	nU := &test{}
	err = protobuf.Decode(data, nU)
	require.NoError(t, err)
	require.Equal(t, val, nU.Val)

	n := &test{Val: val}
	nData, err := protobuf.Encode(n)
	require.NoError(t, err)
	fmt.Printf("%08b\n", nData)

	tU := &proto.Test{}
	err = protobuf.Decode(nData, tU)
	require.NoError(t, err)
	require.Equal(t, val, tU.Value)
}
