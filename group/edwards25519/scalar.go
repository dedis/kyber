...
// The scalar will then be reduced modulo the prime order.
...
func (s *scalar) Bytes() []byte {
	// Take a copy of s.v (type [32]byte) into buf.
	var buf = s.v
	// The s.v internal format is LE.
	encoding.ConvertEndian(buf[:], buf[:])
...
func (s *scalar) UnmarshalBinary(buf []byte) error {
	if len(buf) < 32 {
		return fmt.Errorf("buffer too small: %v < 32", len(buf))
	}
	copy(s.v[:], buf)
	return nil
}
