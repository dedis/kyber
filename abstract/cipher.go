...
// the cipher.State interface, which represents and abstracts over
// an underlying message cipher implementation.
// The underlying cipher.State instance typically embodies
...
// when the Cipher (and its underlying cipher.State) instance is constructed.
...
//
type Cipher struct {
	cipher.State // underlying message cipher implementation
}
...
func (c Cipher) EndMessage() {
	c.State.Message(nil, nil, nil) // finalize the current message
}
...
func (c Cipher) XORKeyStream(dst, src []byte) {
	c.State.Partial(dst[:len(src)], src, nil)
}
