package ed25519

import (
	"encoding/hex"
	"testing"

	"github.com/dedis/crypto/abstract"
	"github.com/dedis/crypto/random"
	"github.com/stretchr/testify/assert"
)

var suite = NewAES128SHA512Ed25519()

func randomSecret() abstract.Secret {
	return suite.Secret().Pick(random.Stream)
}

func TestSecretEqual(t *testing.T) {
	s1 := randomSecret()
	s2 := suite.Secret().Set(s1)

	if !s1.Equal(s2) || !s2.Equal(s1) {
		t.Error("Ed25519 secrets not equal")
	}
}

func TestSecretAddSub(t *testing.T) {
	s := randomSecret()
	s1 := suite.Secret().One()
	add1 := suite.Secret().Add(s, s1)
	sub1 := suite.Secret().Sub(add1, s1)
	if !sub1.Equal(s) {
		t.Error("Add / Sub does not work")
	}
}

func TestSecretMarshal(t *testing.T) {
	s := randomSecret()
	buff, err := s.MarshalBinary()
	assert.Nil(t, err)

	s2 := suite.Secret()
	assert.Nil(t, s2.UnmarshalBinary(buff))

	if !s.Equal(s2) {
		t.Error("Secret marshaling does not work")
	}
}

// FixedStream is used for testing purpose to have a deterministic stream cipher
// which only returns .. its seed ;)
type FixedStream struct {
	seed []byte
}

func (fs *FixedStream) XORKeyStream(dst, src []byte) {
	copy(dst, fs.seed)
}

func TestEdDSAScalarMul(t *testing.T) {
	a := randomSecret()
	b := randomSecret()
	ab := randomSecret()
	ab = ab.Mul(a, b)

	a2 := ab.Div(ab, b)

	if !a.Equal(a2) {
		t.Error("Div not working!")
	}
}

// Testing distributivity for these EdDSA scalars
func TestEdDSAPublicAggregate(t *testing.T) {
	a := randomSecret()
	A := suite.Point().Mul(nil, a)
	b := randomSecret()
	B := suite.Point().Mul(nil, b)

	c := suite.Secret().Add(a, b)
	C := suite.Point().Mul(nil, c)
	AB := suite.Point().Add(A, B)
	if !C.Equal(AB) {
		t.Error("Wrong for Add!")
	}

	d := suite.Secret().Sub(a, b)
	D := suite.Point().Mul(nil, d)

	AMB := suite.Point().Sub(A, B)

	if !D.Equal(AMB) {
		t.Error("Wrong for sub")
	}
}

// EdDSATestVectors taken from the RFC with Ed25519 test vectors with compressed
// public keys
// https://tools.ietf.org/html/draft-josefsson-eddsa-ed25519-02#section-6
var EdDSATestVectors = []struct {
	private   string
	public    string
	message   string
	signature string
}{
	{"9d61b19deffd5a60ba844af492ec2cc44449c5697b326919703bac031cae7f60",
		"d75a980182b10ab7d54bfed3c964073a0ee172f3daa62325af021a68f707511a",
		"",
		"e5564300c360ac729086e2cc806e828a84877f1eb8e5d974d873e065224901555fb8821590a33bacc61e39701cf9b46bd25bf5f0595bbe24655141438e7a100b"},
	{"4ccd089b28ff96da9db6c346ec114e0f5b8a319f35aba624da8cf6ed4fb8a6fb",
		"3d4017c3e843895a92b70aa74d1b7ebc9c982ccf2ec4968cc0cd55f12af4660c",
		"72",
		"92a009a9f0d4cab8720e820b5f642540a2b27b5416503f8fb3762223ebdb69da085ac1e43e15996e458f3613d0f11d8c387b2eaeb4302aeeb00d291612bb0c00"},
	{"c5aa8df43f9f837bedb7442f31dcb7b166d38535076f094b85ce3a2e0b4458f7",
		"fc51cd8e6218a1a38da47ed00230f0580816ed13ba3303ac5deb911548908025",
		"af82",
		"6291d657deec24024827e69c3abe01a30ce548a284743a445e3680d7db5ac3ac18ff9b538d16f290ae67f760984dc6594a7c15e9716ed28dc027beceea1ec40a"},
	{"f5e5767cf153319517630f226876b86c8160cc583bc013744c6bf255f5cc0ee5",
		"278117fc144c72340f67d0f2316e8386ceffbf2b2428c9c51fef7c597f1d426e",
		"",
		"0aab4c900501b3e24d7cdf4663326a3a87df5e4843b2cbdb67cbf6e460fec350aa5371b1508f9f4528ecea23c436d94b5e8fcd4f681e30a6ac00a9704a188a03"},
}

// Comparing our implementation with the test vectors of the RFC
func TestEdDSASigning(t *testing.T) {
	for _, vec := range EdDSATestVectors {
		seed, err := hex.DecodeString(vec.private)
		assert.Nil(t, err)
		stream := FixedStream{
			seed: seed,
		}

		sec := suite.Secret().Pick(&stream)
		pub := suite.Point().Mul(nil, sec)
		data, _ := pub.MarshalBinary()
		if hex.EncodeToString(data) != vec.public {
			t.Error("Public not equal")
		}

		msg, _ := hex.DecodeString(vec.message)

		sig, err := EdDSASign(suite, sec, pub, msg)
		assert.Nil(t, err)

		if hex.EncodeToString(sig) != vec.signature {
			t.Error("Signature wrong")
		}
		assert.Nil(t, EdDSAVerify(suite, pub, msg, sig))
	}
}
